...
<div class="page-content">
    <div class="container">
        <div class="row">
            ...
            @if(!$firstChat)
                <div class="container">
                    <h3 class=" text-center" style="margin-top: 100px;">Belum ada chat tersedia</h3>
                </div>
            @endif
            @if ($firstChat)
                <div class="container">
                                ...
                                @if ($chat->id == $firstChat->id)
                                    <div class="chat_list active_chat">
                                        <div class="chat_people">
                                            ...
                                            <div class="chat_ib">
                                                <a href="{{route('owner.chat.show', $chat->id)}}"><h5>{{$chat->Customer->first_name}} {{$chat->Customer->last_name}}<span class="chat_date">{{$chat->updated_at->format('H:i A, d M Y')}} </span></h5></a>
                                        ...
                                        <div class="chat_people">
                                            <div class="chat_img"> 
                                                ...
                                                @else
                                                    src="{{asset('images/customer/'.$chat->Customer->avatar)}}"
                                                @endif  alt="sunil"  
                                                class="img-circle" width="100" height="50">
                                            </div>
                                            <div class="chat_ib">
                                            <a href="{{route('owner.chat.show', $chat->id)}}"><h5>{{$chat->Customer->first_name}} {{$chat->Customer->last_name}}<span class="chat_date">{{$chat->updated_at->format('H:i A, d M Y')}} </span></h5></a>
                                                <p>{{$chat->lastChat()->message}}</p>
                            ...
                            <div class="mesgs">
                                ...
                                @foreach ($firstChat->chatDetail as $chatDetail)
                                    ...
                                    <div class="incoming_msg">
                                        ...
                                        <img @if ($chatDetail->Customer->Customer->avatar == NULL)
                                            src="{{ asset('templates/img/user.png') }}"
                                        @else
                                            src="{{asset('images/customer/'.$chatDetail->Customer->Customer->avatar)}}"
                                        @endif alt="sunil" class="img-circle" width="100" height="40"> </div>
                                ...
                                <div class="type_msg">
                                    <div class="input_msg_write">
                                        ...
                                        <button class="msg_send_btn" type="button" onclick="sendMessage({{$firstChat->Venue->id}}, {{$firstChat->Customer->id}}, {{Auth::user()->owner->id}})">
